...
// Máxima longitud de una línea (en principio, BUFSIZ, el tamaño del buffer de
// E/S).
...
static Jugadores *busca_usuario(const char *usuario, Jugadores jugador[],
...
/*
    ...
    La ejecución del programa comienza con un mensaje inicial solicitando el
   usuario y contraseña para acceder a ESI-ESCAPE. Los datos introducidos se
   deberán contrastar con los datos previamente cargados en memoria (procedentes
   de Jugadores.txt). Si los datos introducidos no se encuentran en el sistema,
   este debe dar la posibilidad de registrarse con el perfil de jugador, dando
   acceso directo al sistema.
*/

/*
    El enunciado dice que se emplee memoria dinámica, se entiende que cuando sea
   necesario, es decir, cuando no se conozca de antemano el número de objetos
   necesarios. Si el tamaño es fijo y conocido, no tiene sentido (no vamos a
   poner las variables locales en memoria dinámica).

    Para poder añadir un nuevo jugador, necesitamos modificar el vector de
   jugadores y el número de jugadores, pasándolos por referencia, es decir, con
   punteros.

    Parámetros:

    jugadores           (E/S)   Puntero al vector de jugadores en memoria
   dinámica (puntero a puntero a Jugador). número_jugadores    (E)     Puntero
...
Jugadores *login(Jugadores **jugadores, int *numero_jugadores) {
  ...
  while (!sesión_iniciada) {
    ...
    if (jugador) { // O, equivalentemente, jugador != NULL.
      ...
      // El usuario no existe, informamos y damos la opción de registrarse como
      // nuevo usuario.
...
//
// Realiza una búsqueda lineal por nombre de usuario y, si encuentra un jugador
// con ese nombre de usuario, devuelve un puntero a él. En caso contrario,
...
// Valor devuelto:
//
// Puntero al (primer) jugador con ese nombre de usuario o NULL si no existe
// ninguno con ese nombre.

Jugadores *busca_usuario(const char *usuario, Jugadores jugador[],
                         int numero_jugadores) {
  int encontrado = 0;
  int k;
...
// Postcondición:
//
//   cadena contiene los n - 1 primeros caracteres de la entrada seguidos de
//   '\0'
...
void lee_cadena(const char *mensaje, char *cadena, int n) {
  ...
  while (!longitud_correcta) {
    ...
    // Aseguramos que exista un '\0'. Ver el manual: puede no haberlo si fgets
    // falla...
...
void registra_usuario(Jugadores **jugadores, int *numero_jugadores) {
  ...
  while (!respuesta_correcta) {
    lee_cadena(
        "El usuario no existe. ¿Desea registrar un nuevo usuario? [S/N] ",
...
int nuevo_usuario(Jugadores **jugadores, int *número_jugadores) {
  ...
  lee_cadena("Nombre: ", nuevo_jugador.nombre_jugador,
             sizeof nuevo_jugador.nombre_jugador);
  lee_cadena("Usuario: ", nuevo_jugador.id_jugador,
             sizeof nuevo_jugador.id_jugador);
  // Buscamos si ese jugador ya está registrado (solo se comprueba el usuario,
  // puede haber personas con el mismo nombre).
  if (busca_usuario(nuevo_jugador.id_jugador, *jugadores, *número_jugadores)) {
    return 0;
  } else {
    lee_cadena("Contraseña: ", nuevo_jugador.contrasena,
               sizeof nuevo_jugador.contrasena);
    ...
    *jugadores = realloc(
        *jugadores,
        sizeof(Jugadores) *
            *número_jugadores); // Reservamos espacio para una estructura extra.
